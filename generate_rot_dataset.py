...
## 运行示例
  ...
  python generate_rot_dataset.py --input Chambi.json --output Chambi_benchmark.json
  python generate_rot_dataset.py --input Chambi.json --output out.json --start 0 --end 100 --max-items 50

环境变量可兼用 QWEN_MAX_* / DASHSCOPE_API_KEY。
自动依次尝试加载 .env：脚本所在目录、当前工作目录、脚本目录/ethi_ambrot_app、上级/ethi_ambrot_app（不覆盖已 export 的变量）。
...
def main() -> int:
    ...
    env_tried = load_env_for_qwen(here)
